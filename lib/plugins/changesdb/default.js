...
const setup = (dbName)=> {
  return new Promise((resolve, reject) => {
    app.cloudant.db.create(app.dbName+'_changes', (err, body, header) => {
      ...
      const channelindex = {
        index: { fields: ['i_channel', 'i_seq_num'] },
        name: 'channel_index'
      };
      db.createIndex(channelindex);
    ...
    changes.forEach(change => {
      ...
      channels.forEach(channel => {
        ...
        docs.push({
              _id: id,
              seq: change.seq,
              i_seq_num: seq_num, // for indexing
              i_channel: printChangeChannelId(channel), // for indexing
...
const getAllUserChannels = async (user) =>{
  ...
  try {
    const q = {
      selector: {
        '_id': {
          '$gte': printUserUniqueChannel(user.id, ''),
          '$lte': printUserUniqueChannel(user.id, '', true )
          },
        },
       // 'limit': Number(opts.limit||1000)
    };
...
const loadChangesFromCouch = async (user, since = null, limit) => {
  try {
    ...
    for(let i = 0; i < channels.length; i++){
      ...
      /*
      const alldocsq = {
        startkey: printChangeChannelId(channels[i], since),
        endkey: printChangeChannelId(channels[i], null, true),
        limit: 2,
      }
      const q = {
        selector: {
      ...
      */
      const qq = {
        selector: {
          i_channel: { $eq: printChangeChannelId(channels[i]) },
          i_seq_num: { $gte: Number(since) }
        },
        limit: Number(limit)
      }
      /*
      ...
      console.log(q);
      console.log(qq);
      const res2 = await db.list(alldocsq);
      */
      const res3 = await db.find(qq);
      //console.log('res3: ', res3);
      changes = changes.concat(res3.docs);
    }
